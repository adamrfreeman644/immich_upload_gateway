...
BASE='''*{box-sizing:border-box}body{margin:0;min-height:100vh;font-family:system-ui,-apple-system,Segoe UI,sans-serif;background:#f3f4f6;color:#111827}.wrap{width:min(92vw,680px);margin:auto;padding:7vh 0}.card{background:#fff;padding:28px;box-shadow:0 14px 40px #0002}h1{margin:0 0 7px}.muted{color:#6b7280}input[type=file]{width:100%;padding:20px;border:2px dashed #9ca3af;background:#fafafa}.btn{width:100%;border:0;padding:15px;margin-top:14px;font-weight:800;color:white;cursor:pointer}.btn:disabled{cursor:not-allowed;opacity:.6}.status{white-space:pre-wrap;margin-top:13px;font-weight:700}.ok{color:#15803d}.warn{color:#b45309}.bad{color:#b91c1c}.file-list{display:grid;gap:10px;margin-top:16px}.file-row{padding:12px;border:1px solid #d1d5db;border-radius:10px;background:#fff}.file-head{display:flex;align-items:flex-start;gap:10px}.file-name{min-width:0;flex:1;font-weight:700;overflow-wrap:anywhere}.file-state{flex:none;font-size:.82rem;font-weight:800}.file-progress{height:9px;background:#e5e7eb;border-radius:999px;overflow:hidden;margin-top:9px}.file-fill{height:100%;width:0;transition:width .15s ease}.file-reason{display:none;margin:8px 0 0;font-size:.85rem;overflow-wrap:anywhere}.retry{display:none;border:0;border-radius:7px;padding:7px 11px;margin-top:9px;font-weight:800;color:#fff;background:#b91c1c;cursor:pointer}@media(prefers-color-scheme:dark){body{background:#0b1017;color:#f8fafc}.card{background:#141b25}.muted{color:#9ca3af}input[type=file]{background:#0d141e;color:#fff;border-color:#475569}.file-row{background:#0d141e;border-color:#334155}.file-progress{background:#334155}}'''
...
function renderFiles(){{files.replaceChildren();items=[...f.files].map(file=>{{let row=document.createElement('div');row.className='file-row';let head=document.createElement('div');head.className='file-head';let name=document.createElement('div');name.className='file-name';name.textContent=file.name;let state=document.createElement('span');state.className='file-state';state.textContent='Waiting';let meta=document.createElement('div');meta.className='muted';meta.style.fontSize='.8rem';meta.textContent=size(file.size);let progress=document.createElement('div');progress.className='file-progress';let fill=document.createElement('div');fill.className='file-fill';let reason=document.createElement('p');reason.className='file-reason';let retry=document.createElement('button');retry.type='button';retry.className='retry';retry.textContent='Retry';head.append(name,state);progress.append(fill);row.append(head,meta,progress,reason,retry);files.append(row);let item={{file,state,fill,reason,retry,status:'waiting'}};retry.onclick=async()=>{{retry.disabled=true;await uploadOne(item);summary()}};return item}});s.textContent=items.length?`${{items.length}} file${{items.length===1?'':'s'}} ready to upload.`:'';s.className='status'}}
